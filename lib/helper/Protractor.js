...
class Protractor extends Helper {
  ...
  _before() {
    ...
    this.protractorBrowser = this.browser = protractor.wrapDriver(browser, this.options.url, this.options.rootElement);       
   ...
   */
  amOutsideAngularApp() {
     this.browser = this.webdriverBrowser
  }
  
  amInsideAngularApp() {
    this.browser = this.protractorBrowser; 
  }
  ...
  seeElement(locator) {
    return this.browser.findElements(guessLocator(locator) || by.css(locator)).then((els) => {
      return empty('elements').negate(els.length);
    });
  }
  ...
  dontSeeElement(locator) {
    return this.browser.findElements(guessLocator(locator) || by.css(locator)).then((els) => {
      return empty('elements').assert(els.length);
    });
  }
...
function proceedSee(assertType, text, context) {
  ...
  return this.browser.findElement(guessLocator(context) || by.css(context)).getText().then(function (source) {
